...
internal class CoreHookService : IDisposable
{
    ...
    */
    ...
    private delegate nint FindConCommandWindows(
        nint pICvar,
        nint pRet,
        nint pConCommandName,
        int unk1
    );
    private delegate nint FindConCommandLinux( nint pICvar, nint pConCommandName, int unk1 );
    private delegate int CCSPlayerItemServicesCanAcquire(
        nint pItemServices,
        nint pEconItemView,
        nint acquireMethod,
        nint unk1
    );
    private delegate byte CCSPlayerWeaponServicesCanUse(
        nint pWeaponServices,
        nint pBasePlayerWeapon
    );
    ...
    private void HookExecuteCommand()
    {
        ...
        executeCommandGuid = executeCommand.AddHook(
            ( next ) =>
            {
                return ( a1, a2, a3, a4, a5 ) =>
                {
                    unsafe
                    {
                        if (a5 != nint.Zero)
                        {
                            ...
                            var @eventPre = new OnCommandExecuteHookEvent(
                                ref command,
                                HookMode.Pre
                            );
                            ...
                            var @eventPost = new OnCommandExecuteHookEvent(
                                ref command,
                                HookMode.Post
                            );
    ...
    private void HookFindConCommandTemplate()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            ...
            findConCommandLinux = core.Memory.GetUnmanagedFunctionByVTable<FindConCommandLinux>(
                core.Memory.GetVTableAddress("tier0", "CCvar")!.Value,
                offset
            );

            logger.LogInformation(
                "Hooking ICvar::FindConCommand at {Address}",
                findConCommandLinux.Address
            );

            findConCommandGuid = findConCommandLinux.AddHook(
                ( next ) =>
                {
        ...
        {
            var offset = core.GameData.GetOffset("ICvar::FindConCommand");
            findConCommandWindows = core.Memory.GetUnmanagedFunctionByVTable<FindConCommandWindows>(
                core.Memory.GetVTableAddress("tier0", "CCvar")!.Value,
                offset
            );
            ...
            logger.LogInformation(
                "Hooking ICvar::FindConCommand at {Address}",
                findConCommandWindows.Address
            );

            findConCommandGuid = findConCommandWindows.AddHook(
                ( next ) =>
                {
    ...
    private void HookCCSPlayerItemServicesCanAcquire()
    {
        ...
        itemServicesCanAcquire =
            core.Memory.GetUnmanagedFunctionByAddress<CCSPlayerItemServicesCanAcquire>(address);
        itemServicesCanAcquireGuid = itemServicesCanAcquire.AddHook(next =>
        {
            return ( pItemServices, pEconItemView, acquireMethod, unk1 ) =>
            {
                ...
                var @event = new OnItemServicesCanAcquireHookEvent {
                    ...
                    WeaponVData = core.Helpers.GetWeaponCSDataFromKey(
                        econItemView.ItemDefinitionIndex
                    ),
    ...
    private void HookCCSPlayerWeaponServicesCanUse()
    {
        ...
        weaponServicesCanUse =
            core.Memory.GetUnmanagedFunctionByVTable<CCSPlayerWeaponServicesCanUse>(
                pVtable!.Value,
                offset
            );
        logger.LogInformation(
            "Hooking CCSPlayer_WeaponServices::CanUse at {Address}",
            weaponServicesCanUse.Address
        );
    ...
    private void HookCBaseEntityTouchTemplate()
    {
        ...
        entityStartTouch = core.Memory.GetUnmanagedFunctionByVTable<CBaseEntityTouchTemplate>(
            pVtable!.Value,
            startTouchOffset
        );
        entityTouch = core.Memory.GetUnmanagedFunctionByVTable<CBaseEntityTouchTemplate>(
            pVtable!.Value,
            touchOffset
        );
        entityEndTouch = core.Memory.GetUnmanagedFunctionByVTable<CBaseEntityTouchTemplate>(
            pVtable!.Value,
            endTouchOffset
        );
        logger.LogInformation(
            "Hooking CBaseEntity::StartTouch at {Address}",
            entityStartTouch.Address
        );
        ...
        entityStartTouchGuid = entityStartTouch.AddHook(next =>
        {
            return ( pBaseEntity, pOtherEntity ) =>
            {
                ...
                EventPublisher.InvokeOnEntityStartTouch(
                    new OnEntityStartTouchEvent { Entity = entity, OtherEntity = otherEntity }
                );
                EventPublisher.InvokeOnEntityTouchHook(
                    new OnEntityTouchHookEvent {
                        Entity = entity,
                        OtherEntity = otherEntity,
                        TouchType = EntityTouchType.StartTouch,
                    }
                );
        ...
        entityTouchGuid = entityTouch.AddHook(next =>
        {
            return ( pBaseEntity, pOtherEntity ) =>
            {
                var entity = new CBaseEntityImpl(pBaseEntity);
                var otherEntity = new CBaseEntityImpl(pOtherEntity);
                EventPublisher.InvokeOnEntityTouch(
                    new OnEntityTouchEvent { Entity = entity, OtherEntity = otherEntity }
                );
                EventPublisher.InvokeOnEntityTouchHook(
                    new OnEntityTouchHookEvent {
                        Entity = entity,
                        OtherEntity = otherEntity,
                        TouchType = EntityTouchType.Touch,
                    }
                );
                return next()(pBaseEntity, pOtherEntity);
            };
        ...
        entityEndTouchGuid = entityEndTouch.AddHook(next =>
        {
            return ( pBaseEntity, pOtherEntity ) =>
            {
                var entity = new CBaseEntityImpl(pBaseEntity);
                var otherEntity = new CBaseEntityImpl(pOtherEntity);
                EventPublisher.InvokeOnEntityEndTouch(
                    new OnEntityEndTouchEvent { Entity = entity, OtherEntity = otherEntity }
                );
                EventPublisher.InvokeOnEntityTouchHook(
                    new OnEntityTouchHookEvent {
                        Entity = entity,
                        OtherEntity = otherEntity,
                        TouchType = EntityTouchType.EndTouch,
                    }
                );
                return next()(pBaseEntity, pOtherEntity);
            };
    ...
    private void HookSteamServerAPIActivated()
    {
        ...
        steamServerAPIActivated = core.Memory.GetUnmanagedFunctionByVTable<SteamServerAPIActivated>(
            pVtable!.Value,
            offset
        );
        logger.LogInformation(
            "Hooking IServerGameDLL::GameServerSteamAPIActivated at {Address}",
            steamServerAPIActivated.Address
        );
    ...
    private void HookCPlayerMovementServicesRunCommand()
    {
        ...
        movementServiceRunCommand =
            core.Memory.GetUnmanagedFunctionByVTable<CPlayerMovementServicesRunCommand>(
                pVtable!.Value,
                offset
            );
        logger.LogInformation(
            "Hooking CPlayer_MovementServices::RunCommand at {Address}",
            movementServiceRunCommand.Address
        );
        movementServiceRunCommandGuid = movementServiceRunCommand.AddHook(
            ( next ) =>
            {
    ...
    private void HookCCSPlayerPawnPostThink()
    {
        ...
        playerPawnPostThink = core.Memory.GetUnmanagedFunctionByAddress<CCSPlayerPawnPostThink>(
            address
        );
        playerPawnPostThinkGuid = playerPawnPostThink.AddHook(
            ( next ) =>
            {
                return ( pPlayerPawn ) =>
                {
                    ...
                    var @event = new OnPlayerPawnPostThinkHookEvent { PlayerPawn = playerPawn };
