...
internal class ProfileService
{
    ...
    public string GenerateJSONPerformance( string pluginId )
    {
      ...
      { "ts", 0UL },
        ...
        static string FormatUs( float us )
        {
        ...
        foreach (var (plugin, nameMap) in stats)
        {
            ...
            foreach (var (name, stat) in nameMap)
            {
                ...
                var eventName = $"{name} [{plugin}] (min={FormatUs(minUs)},avg={FormatUs(avgUs)},max={FormatUs(maxUs)},count={count})";
