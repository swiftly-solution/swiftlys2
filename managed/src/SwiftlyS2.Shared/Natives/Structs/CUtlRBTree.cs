    ...
    where TKey : unmanaged, IBinaryInteger<TKey>, IMinMaxValue<TKey>
{
    ...
    private int Depth( TKey i )
    {
        ...
        var leftDepth = Depth(LeftChild(i));
        var rightDepth = Depth(RightChild(i));
    ...
    public void InsertRebalance( TKey elem )
    {
        ...
        while (elem != Root && IsRed(Parent(elem)))
        {
            ...
            {
                var uncle = RightChild(grandparent);
            ...
            {
                var uncle = LeftChild(grandparent);
                if (IsValidIndex(uncle) && IsRed(uncle))
                {
                    SetColor(parent, NodeColor_t.BLACK);
                    SetColor(uncle, NodeColor_t.BLACK);
    ...
    public void Unlink( TKey elem )
    {
        if (elem != InvalidIndex())
        {
            ...
            x = LeftChild(y) != InvalidIndex() ? LeftChild(y) : RightChild(y);
    ...
    public void Link( TKey elem )
    {
        if (elem != InvalidIndex())
        {
            FindInsertionPosition(this[elem], out var parent, out var leftchild);

            LinkToParent(elem, parent, leftchild);
        }
    }

    private void FindInsertionPosition( TValue val, out TKey parent, out bool leftchild )
    {
        parent = InvalidIndex();
        leftchild = false;
    ...
    {
        FindInsertionPosition(val, out var parent, out var leftchild);

        var newNode = InsertAt(parent, leftchild);
        this[newNode] = val;
        return newNode;
    }
