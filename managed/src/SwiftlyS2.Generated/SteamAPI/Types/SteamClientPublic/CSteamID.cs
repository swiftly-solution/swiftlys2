...
public partial class SteamIdParser
{
    ...
    public static ulong? ParseToSteamId64( string input )
    {
        ...
        return TryParseSteamId3(input, out steamId64) ? steamId64 : null;
    }
    ...
    private static bool TryParseSteamId64( string input, out ulong steamId64 )
    {
        ...
        return !SteamId64Regex.IsMatch(input) ? false : ulong.TryParse(input, out steamId64) && steamId64 >= STEAM_ID_BASE;
    }
    ...
    private static bool TryParseSteamId( string input, out ulong steamId64 )
    {
        ...
        if (!uint.TryParse(match.Groups[1].Value, out var y))
            return false;

        if (!uint.TryParse(match.Groups[2].Value, out var z))
            return false;
    ...
    private static bool TryParseSteamId3( string input, out ulong steamId64 )
    {
        ...
        if (!uint.TryParse(match.Groups[1].Value, out var accountId))
            return false;
